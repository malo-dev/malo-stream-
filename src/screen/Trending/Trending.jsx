...
const Trending = () => {
  ...
  const [artist,setartist]=useState("dadju")
    ...
    const searchFunction = () => {
      ...
      fetch(`https://api.spotify.com/v1/search?q=${artist}&type=track%2Cartist&market=ES&limit=30&offset=0`, Settings).then(res => res.json())
        .then(data => {
          
          setAlbum(data.artists?.items[0].id)
